...
[`routerAdd()`](/jsvm/functions/routerAdd.html) and
[`routerUse()`](/jsvm/functions/routerUse.html) functions.
...
URL then you need to end the path with the special `{$}` parameter.**

> **Info:** If your route path starts with `/api/` consider combining it with your unique app name like `/api/myapp/...` to avoid collisions with system routes.
...
to specify a custom priority and id - as a [`Middleware`](/jsvm/classes/Middleware.html) class instance.
...
- **Activity logger** _(id: pbActivityLogger, priority: -1040)_
  Saves request information into the logs auxiliary database.

- **Auto panic recover** _(id: pbPanicRecover, priority: -1030)_
  Default panic-recover handler.

- **Auth token loader** _(id: pbLoadAuthToken, priority: -1020)_
...
- **Security response headers** _(id: pbSecurityHeaders, priority: -1010)_
...
[`$apis.recordAuthResponse()`](/jsvm/functions/_apis.recordAuthResponse.html) writes standardized JSON record auth response (aka. token + record data) into the specified request body. Could
...
[`$apis.enrichRecord()`](/jsvm/functions/_apis.enrichRecord.html) and
